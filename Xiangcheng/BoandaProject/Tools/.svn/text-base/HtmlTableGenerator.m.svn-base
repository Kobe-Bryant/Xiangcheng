...
+(NSString*)genContentWithTitle1:(NSString*)aTitle andParaMeters:(NSDictionary*)params
                        andType:(NSString*)serviceKey{
    ...
    for (int i = 0; i < count; ) {
        ...
        if (i+1 < count) {
            ...
            if([keyType intValue] == 2 && [nextKeyType intValue] == 2){
                ...
                if(value1 == nil)value1 = @"";
                if(value2 == nil)value2 = @"";
                [paramsStr appendFormat:@"<tr><td width=\"18%%\" class=\"t1\">%@</td><td  width=\"32%%\"  class=\"t2\">%@</td><td width=\"18%%\" class=\"t1\">%@</td><td width=\"32%%\"  class=\"t2\">%@</td></tr>",
                 [tmpDic objectForKey:key1],value1,[tmpDic objectForKey:key2],value2];
                i+=2;
                continue;
            }
        ...
        NSString *value = [params objectForKey:key1];
        if(value == nil)value = @"";
        [paramsStr appendFormat:@"<tr><td width=\"18%%\" class=\"t1\">%@</td><td colspan=\"3\" width=\"82%%\"  class=\"t2\">%@</td></tr>",
         [tmpDic objectForKey:key1],value];
...
+(NSArray*)genContentWithValue:(NSDictionary*)params{
    
    NSDictionary *configDic = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"json" ofType:@"plist"]];
    
    NSDictionary * tmpDic = [configDic objectForKey:@"XGJLNUMBER"];
    NSArray *aryKeys = [tmpDic objectForKey:@"keys"];
    NSMutableArray *array = [NSMutableArray array];
    for (NSString *key in aryKeys) {
        NSMutableDictionary *value = [NSMutableDictionary dictionary];
        [value setObject:[tmpDic objectForKey:key] forKey:@"title"];
        [value setObject:[params objectForKey:key] forKey:@"number"];
